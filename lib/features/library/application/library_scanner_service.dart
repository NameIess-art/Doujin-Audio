...
class LibraryScannerService {
  ...
  Future<LocalLibraryImportSources?> prepareBackupRestoreSources({
    ...
    Future<List<String>?> prepareFolders(
      ...
      return _distinctSourcePaths(prepared);
    }

    final requestedLibraries = resolvedLibraries;
    final requestedFolders = _distinctSourcePaths(
      resolvedFolders.where(
        (folder) => !requestedLibraries.any(
          (library) => PathMatcher.isWithinOrEqual(folder, library),
        ),
      ),
    );
    final preparedLibraries = await prepareFolders(
      requestedLibraries,
      labels.chooseLibraryFolder,
    );
    if (preparedLibraries == null) return null;
    final preparedFolderValues = await prepareFolders(
      requestedFolders,
      labels.chooseMusicFolder,
    );
    if (preparedFolderValues == null) return null;
    final preparedFolders = _distinctSourcePaths(
      preparedFolderValues.where(
        (folder) => !preparedLibraries.any(
          (library) => PathMatcher.isWithinOrEqual(folder, library),
        ),
      ),
    );
    final preparedFiles = <String>[];
    var needsFilePicker = false;
    for (final raw in resolvedFiles) {
      final source = raw.trim();
      if (source.isEmpty) continue;
      if (!PathMatcher.isContentUri(source)) {
        if (fileSystemPermissionGranted &&
    ...
    if (needsFilePicker) {
      final replacements = await _dataSource.pickAudioFiles(
    ...
    return LocalLibraryImportSources(
      ...
      files: _distinctSourcePaths(preparedFiles),
    );
  }

  Future<LibraryScanOutcome> restoreSources({
    required LocalLibraryImportSources sources,
    required LibraryCatalog provider,
    required LibraryScanLabels labels,
  }) async {
    ...
    void record(LibraryScanOutcome outcome) {
      added += outcome.addedCount;
      if (_isRestoreFailure(outcome.code)) {
        failureCount++;
      } else {
        restoredSourceCount++;
      }
    ...
    for (final libraryPath in sources.libraries) {
      record(await _addLibraryFromPath(libraryPath, provider, labels));
    }
    for (final folderPath in sources.folders) {
      record(await _addFolderFromPath(folderPath, provider, labels));
    }
    if (sources.files.isNotEmpty) {
      record(
        await addFilePaths(sources.files, provider: provider, labels: labels),
      );
    }
    return LibraryScanOutcome(
      code: failureCount == 0
          ? LibraryScanOutcomeCode.importAdded
          : LibraryScanOutcomeCode.failed,
      source: 'restore_backup_sources',
      details: <String, Object?>{
        'count': added,
        'sourceCount': restoredSourceCount,
