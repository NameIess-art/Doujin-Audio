...
- 播放列表支持长按进入批量选择，可批量播放、暂停或移除会话；多选播放会遵循多会话播放设置。从播放队列中移除当前正在播放的曲目时，继续播放当前曲目直至自然播放完毕或手动切歌，不中断当前收听。
...
- 播放速度：支持 0.25x 至 3.0x 范围调节，提供常用快捷倍速预设与平滑微调滑块。
...
- 根据文件树构建层级媒体库，支持按名称、声优、时长、发售时间或添加时间排序，也支持搜索、刷新扫描、排除目录和手动拖动排序。媒体库搜索支持批量选择模式，可长按多选条目快速加入播放列表或管理。
...
- 已移除目录管理：从媒体库中永久移除的文件夹支持持久化保存，可在单独的“已移除目录”管理界面中随时查看或恢复；移除操作附带撤销过渡提醒。
...
- 支持按 RJ 号、文件名或作品标题读取 DLsite 元数据，支持批量匹配与写入，并在审查页面提供多作品快速比对与切换导航。

### 视频转音频

- 从本地选择视频文件和输出目录，将视频音轨转换为 MP3、AAC、OGG、WAV 或 FLAC；持久化记忆上次选择的输出目录。
...
- 设置页面提供粘性分类标题（Sticky Section Headers），支持内联卡片查看并配置存储与通知权限。可选择启动页（ASMR.ONE、本地音频库或播放列表）、底部导航样式、竖屏锁定、操作震动和减少动画；页面、对话框与底部面板复用统一的方向切换和淡入淡出动效，并遵循减少动画设置。
