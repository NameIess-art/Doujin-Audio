...
extension AudioProviderAudioDetails on AudioProvider {
  ...
  Future<AudioDetailSaveResult> saveAudioDetail(AudioDetail detail) async {
    return _saveAudioDetail(detail);
  }

  Future<AudioDetailSaveResult> _saveAudioDetail(
    AudioDetail detail, {
    bool notify = true,
  }) async {
  ...
  Future<void> backfillMissingLibraryDurations({
    ...
    final inFlight = _missingLibraryDurationBackfill;
    if (inFlight != null) {
      _missingLibraryDurationBackfillRequestedAgain = true;
      return inFlight;
    }

    final task = () async {
      do {
        _missingLibraryDurationBackfillRequestedAgain = false;
        await _backfillMissingLibraryDurations(durationReader: durationReader);
      } while (_missingLibraryDurationBackfillRequestedAgain && !_isDisposed);
    }();
    _missingLibraryDurationBackfill = task;
    unawaited(
      task.then<void>(
        (_) => _clearMissingLibraryDurationBackfill(task),
        onError: (Object error, StackTrace stackTrace) {
          _clearMissingLibraryDurationBackfill(task);
        },
      ),
    );
    return task;
  ...
  void _clearMissingLibraryDurationBackfill(Future<void> task) {
    if (identical(_missingLibraryDurationBackfill, task)) {
      _missingLibraryDurationBackfill = null;
    }
  }
  ...
  Future<void> _backfillMissingLibraryDurations({
    ...
    for (final track in List<MusicTrack>.of(_library)) {
    ...
    final loadResults = await _audioDetailCacheService.loadMany(targets);
    var attemptedDurationProbe = false;
    for (var index = 0; index < targets.length; index++) {
      ...
      attemptedDurationProbe = true;
      final duration = await _calculateLibraryDurationForTracks(
        targetTracks,
        durationReader: durationReader,
        notify: false,
      );
      if (duration == null || _isDisposed) continue;
      if (detail.duration != null) continue;
      final latestDetail =
          _audioDetailCacheService.resolvedDetail(detail.target) ?? detail;
      if (latestDetail.duration != null) continue;
      await _saveAudioDetail(
        latestDetail.copyWith(duration: duration),
        notify: false,
      );
    }

    if (!attemptedDurationProbe || _isDisposed) return;
    void notifyAfterBackfill() {
      if (!_isDisposed) _notifyListeners();
    }

    if (UiInteractionCoordinator.instance.isInteracting) {
      UiInteractionCoordinator.instance.scheduleCommit(
        key: 'library_duration_backfill_notify',
        priority: 20,
        commit: notifyAfterBackfill,
      );
    } else {
      notifyAfterBackfill();
    }
