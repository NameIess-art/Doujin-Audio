...
- Before upgrading the upstream version, compare the fork against the new upstream release, reapply only required local patches, then run `flutter analyze`, `flutter test`, and `flutter build apk --debug`.
