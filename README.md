...
- 支持 `.nalbackup` 数据备份、验证、恢复和失败回滚；导出前会完整落盘所有可持久化设置页选项。本地媒体库只备份原始文件夹、文件和曲库路径，恢复时按这些路径重新导入并按需请求目录权限，避免恢复扫描缓存等派生脏数据；恢复失败会自动回滚到当前数据。
- `.nalbackup` 包含应用偏好、媒体库数据以及用于恢复登录的 ASMR.ONE 用户名、密码和 JWT/token，不包含播放列表。账号凭据以明文写入备份文件，请仅保存到可信位置；恢复后会使用备份中的账号状态自动恢复登录。
