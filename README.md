...
2. 在“本地音频库”中通过 Android 系统文件选择器添加曲库、文件夹或单个文件。
...
- 播放范围：支持单曲循环、当前文件夹顺序/随机、跨文件夹顺序/随机，以及播放到范围末尾后暂停的“顺序播放”。
...
- 支持添加文件夹、曲库和单独音频文件；Android 仅使用 SAF，并持久化用户选定来源的访问授权。
- 从旧版本升级后，已有文件系统路径会优先匹配已持久化的 SAF 授权；无法匹配的来源会在曲库管理页提示重新授权，原数据不会被静默删除。
...
- ASMR.ONE 下载任务支持暂停和继续；单个文件遇到瞬时网络错误、响应截断或服务端临时错误时会自动重试最多 5 次，并优先从已有临时文件断点续传。下载任务页和文件详情会显示当前重试次数。
...
- 支持 `.nalbackup` 数据备份、验证、恢复和失败回滚；导出前会完整落盘所有可持久化设置页选项。本地媒体库只备份原始文件夹、文件和曲库来源，Android 恢复旧文件系统来源时会要求用户通过系统选择器重新授权；取消授权会中止替换并保留当前数据库。
- 普通 `.nalbackup` 包含应用偏好、媒体库数据，以及 ASMR.ONE 用户名、密码和 JWT/token 的明文副本；备份 ZIP 本身不加密，恢复后会恢复这些凭据。请仅将备份保存在可信位置并谨慎分享。
...
Android 的曲库、单文件导入和备份恢复均使用系统 Storage Access Framework（SAF）返回并持久化的 `content://` URI，不申请广泛存储或媒体读取权限。Windows 等非 Android 平台仍使用普通文件系统路径。
