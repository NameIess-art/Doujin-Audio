...
public class AudioService extends MediaBrowserServiceCompat {
    ...
    private static final String EXTRA_ANDROID_GROUP_SUMMARY = "android_group_summary";
    private static final String EXTRA_ANDROID_GROUP_KEY = "android_group_key";
    private static final String EXTRA_ANDROID_SUMMARY_TITLE = "android_summary_title";
    private static final String EXTRA_ANDROID_SUMMARY_TEXT = "android_summary_text";
    private static final String EXTRA_ANDROID_SUMMARY_LINES = "android_summary_lines";
    ...
    public static MediaSessionCompat.Token getMediaSessionTokenCompat() {
        return instance != null && instance.mediaSession != null
                ? instance.mediaSession.getSessionToken()
                : null;
    }

    public static void dispatchCustomAction(final String action, final Bundle extras) {
        if (listener == null || action == null) return;
        new Handler(Looper.getMainLooper()).post(() -> listener.onCustomAction(action, extras));
    }
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        if (intent != null && intent.getAction() != null) {
            ...
            if ("toggle_session_playback".equals(action)
                    || "session_skip_previous".equals(action)
                    || "session_skip_next".equals(action)) {
                dispatchCustomAction(action, intent.getExtras());
    ...
    private Notification buildNotification() {
        ...
        final String notificationGroupKey = mediaMetadata != null
                ? mediaMetadata.getString(EXTRA_ANDROID_GROUP_KEY)
                : null;
        if (mediaMetadata != null) {
        ...
        if (isGroupedSummaryNotification()) {
            return buildGroupedSummaryNotification(builder);
        }
        if (notificationGroupKey != null) {
            builder.setGroup(notificationGroupKey);
            builder.setGroupAlertBehavior(NotificationCompat.GROUP_ALERT_SUMMARY);
            builder.setSortKey("1_primary");
        }
    ...
    private boolean isGroupedSummaryNotification() {
        return mediaMetadata != null
                && mediaMetadata.getLong(EXTRA_ANDROID_GROUP_SUMMARY) == 1L
                && mediaMetadata.getString(EXTRA_ANDROID_GROUP_KEY) != null;
    }

    private Notification buildGroupedSummaryNotification(NotificationCompat.Builder builder) {
        final String groupKey = mediaMetadata.getString(EXTRA_ANDROID_GROUP_KEY);
        final String summaryTitle = mediaMetadata.getString(EXTRA_ANDROID_SUMMARY_TITLE);
        final String summaryText = mediaMetadata.getString(EXTRA_ANDROID_SUMMARY_TEXT);
        final String summaryLines = mediaMetadata.getString(EXTRA_ANDROID_SUMMARY_LINES);
        ...
        if (summaryLines != null && !summaryLines.isEmpty()) {
            for (String line : summaryLines.split("\\n")) {
                if (!line.isEmpty()) {
