...
- **音轨播放完毕后停止**：睡眠定时器新增与音轨播放完成状态联动，定时器自动恢复时等待当前曲目自然结束，避免中途突兀切断。
...
- **播放核心与命令统一**：精简播放底层状态流，播放命令与通知栏状态同步完全统一至中央协调器，降低后台功耗与资源占用。
- **动画与过渡性能**：优化底部弹窗（Bottom Sheet）与水波纹遮罩过渡动画，消除多组件动画冲突。
