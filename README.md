...
最新发布页：[v1.1.13](https://github.com/NameIess-art/AudioPlayer/releases/tag/v1.1.13)
...
从 [GitHub Release v1.1.13](https://github.com/NameIess-art/AudioPlayer/releases/tag/v1.1.13) 下载适合设备 CPU 架构的 APK：
...
- 封面图片统一改为填满显示区域：最大化显示，可裁切，不拉伸。
- 优化重装后大曲库扫描/导入卡顿：扫描进度节流，曲目导入批量重建索引、批量写入 SQLite。
- 启动后的封面/字幕预热改为延迟执行并减少预热数量，避免首屏后持续抢占 IO。
- `AudioProvider` 继续收敛：新增 `LibraryController`、`PlaybackSessionController`、`TimerController`、`NotificationCoordinator`，Provider 负责组合状态和控制器。
- 同步版本、README 和 Release 信息到 `1.1.13+10113`，发布 arm64-v8a、armeabi-v7a、x86_64 三个 APK。
